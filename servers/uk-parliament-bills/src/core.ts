...
function enforceEnumIfPresent(parameter: EndpointParameter, parsedValue: string | number | boolean | string[] | number[]): void {
  ...
  const allowed = new Set(parameter.enum.map((entry: string | number | boolean) => String(entry)));
...
export async function executeEndpoint(endpoint: EndpointDefinition, rawInput: Record<string, unknown>, config: RuntimeConfig): Promise<StandardEnvelope> {
  ...
  return {
    ...
    data: normalized,
    upstream_path: decodePath(upstreamUrl.pathname),
...
export function createStandardEnvelopeForHttp(
  ...
  return {
    status,
    data,
    upstream_path: decodeURIComponent(toResolvedPathWithoutQuery(upstreamPath)),
...
export function runtimeConfigFromEnv(): RuntimeConfig {
  return {
    apiBaseUrl: process.env.UPSTREAM_BASE_URL ?? 'https://bills-api.parliament.uk',
  ...
  if (error instanceof UpstreamHttpError) {
    return {
      ...
      body: {
        ...
        upstream_path: decodePath(fallbackPath),
        retrieved_at: retrievedAt,
      },
    };
  }
...
export function matchEndpointByResolvedPath(pathname: string): MatchedEndpoint | undefined {
  ...
  for (const endpoint of ENDPOINTS) {
    ...
    const staticScore = endpoint.path.replace(/\{[^}]+\}/g, '').length;
    const score = staticScore * 1000 - names.length;
  ...
  if (bestMatch) {
    ...
    bestMatch.names.forEach((name, index) => {
      const captured = bestMatch?.captures[index];
