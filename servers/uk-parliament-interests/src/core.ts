...
export async function executeEndpoint(endpoint: EndpointDefinition, rawInput: Record<string, unknown>, config: RuntimeConfig): Promise<StandardEnvelope> {
  ...
  try {
    response = await fetch(upstreamUrl, {
      ...
      headers: {
        accept: 'application/json, application/pdf, application/zip, application/octet-stream, text/plain;q=0.9, */*;q=0.8',
      },
...
export function runtimeConfigFromEnv(): RuntimeConfig {
  return {
    apiBaseUrl: process.env.UKPI_API_BASE_URL ?? 'https://interests-api.parliament.uk',
    requestTimeoutMs: Number(process.env.UKPI_REQUEST_TIMEOUT_MS ?? '10000'),
  };
}
