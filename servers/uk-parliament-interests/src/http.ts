...
export async function startHttpServer(port = Number(process.env.UKPI_HTTP_PORT ?? '8787')): Promise<void> {
    ...
    if (url.pathname.startsWith('/proxy/')) {
      if (method !== 'GET') {
        writeJsonResponse(res, 405, {
          ...
          error: {
            code: 'INVALID_PARAMS',
            message: 'Only GET is supported for proxied UK Parliament Interests endpoints.',
          },
          upstream_path: '',
          retrieved_at: new Date().toISOString(),
        });
  ...
  await new Promise<void>((resolve) => {
    server.listen(port, () => {
      console.error(`UK Parliament Interests HTTP proxy listening on :${port}`);
      resolve();
    });
  });
}
