...
describe('tools registration', () => {
  ...
  it('maps upstream timeout to error payload', async () => {
    ...
    registerAllTools(fakeServer);
    const tool = registered.list_treaties;
  ...
  it('includes enum options from resolved refs in input schema metadata', () => {
    const endpoint = ENDPOINTS.find((entry) => entry.toolName === 'list_treaties');
  ...
  it('keeps required/optional schema mapping from endpoint metadata', () => {
    ...
    const idConfig = configs.get_business_item_by_id as { inputSchema?: z.ZodTypeAny } | undefined;
    const listConfig = configs.list_treaties as { inputSchema?: z.ZodTypeAny } | undefined;
