...
describe('parameter passthrough', () => {
  it('builds query strings and path params from raw input', () => {
    const endpoint = ENDPOINTS_BY_TOOL_NAME.get('get_treaty_by_id');
    ...
    expect(upstreamPath).toBe('/api/Treaty/treaty-123');
  });

  it('throws on invalid enum query param', () => {
    const endpoint = ENDPOINTS_BY_TOOL_NAME.get('list_treaties');
    expect(endpoint).toBeDefined();
  ...
  it('returns 404 for unknown proxy endpoint with available list', async () => {
    ...
    expect(payload.error.details?.available_endpoints?.length).toBe(6);
  });
  ...
  it('maps upstream timeout to 504 via HTTP proxy', async () => {
    ...
    const response = await requestJson(address.port, '/proxy/api/Treaty');
  ...
  it('maps path params through HTTP proxy route', async () => {
    ...
    const response = await requestJson(address.port, '/proxy/api/Treaty/abc123');
    expect(response.statusCode).toBe(200);
    ...
    expect(payload.upstream_path).toBe('/api/Treaty/abc123');
    expect(payload.data.ok).toBe(true);

    const fetchMock = globalThis.fetch as unknown as { mock: { calls: unknown[][] } };
    const calledUrl = String(fetchMock.mock.calls[0]?.[0] ?? '');
    expect(calledUrl).toContain('/api/Treaty/abc123');
