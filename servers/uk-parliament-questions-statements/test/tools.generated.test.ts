...
describe('tools registration', () => {
  ...
  it('maps upstream timeout to error payload', async () => {
    ...
    registerAllTools(fakeServer);
    const tool = registered.list_written_questions;
  ...
  it('includes parameter description and enum options in tool input schema metadata', () => {
    const endpoint = ENDPOINTS.find((entry) => entry.toolName === 'list_written_questions');
    ...
    const statusSchema = properties.questionStatus as { description?: string } | undefined;

    expect(statusSchema?.description).toContain('status specified');
    const enumOptions = collectStringEnumOptions(statusSchema);
    expect(new Set(enumOptions)).toEqual(new Set(['NotAnswered', 'AnsweredOnly', 'AllQuestions']));
  });
  ...
  it('rejects invalid enum input as INVALID_PARAMS with 400', async () => {
    ...
    const result = (await tool?.({ questionStatus: 'NotReal' })) as {
      isError: boolean;
  ...
  it('keeps required/optional schema mapping from endpoint metadata', () => {
    ...
    const idConfig = configs.get_written_question_by_id as { inputSchema?: z.ZodTypeAny } | undefined;
    const listConfig = configs.list_written_questions as { inputSchema?: z.ZodTypeAny } | undefined;
