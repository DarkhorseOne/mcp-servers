...
export async function startHttpServer(port = Number(process.env.UKP_HTTP_PORT ?? '3000')): Promise<import('node:http').Server> {
  ...
  await new Promise<void>((resolve) => {
    server.listen(port, () => {
      console.error(`UK Parliament Erskine May HTTP proxy listening on :${port}`);
      resolve();
    });
  });
