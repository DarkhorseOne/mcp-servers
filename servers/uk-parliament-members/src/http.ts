...
export async function startHttpServer(port = Number(process.env.UKP_HTTP_PORT ?? '8787')): Promise<void> {
  ...
  await new Promise<void>((resolve) => {
    server.listen(port, () => {
      console.error(`UK Parliament Members HTTP proxy listening on :${port}`);
      resolve();
    });
  });
}
