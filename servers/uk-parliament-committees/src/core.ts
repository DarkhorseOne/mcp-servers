...
function normalizeUpstreamData(contentType: string | null, rawBody: string, rawBytes: Uint8Array): unknown {
  ...
  if (
    ...
    return {
      ...
      data_base64: Buffer.from(rawBytes).toString('base64'),
    };
  }
...
export async function executeEndpoint(
  ...
  try {
    response = await fetch(upstreamUrl, {
      ...
      headers: {
        accept: 'application/json, text/json;q=0.9, application/pdf, application/zip, application/octet-stream, text/plain;q=0.8, */*;q=0.7',
      },
  ...
  const normalized = normalizeUpstreamData(response.headers.get('content-type'), rawBody, rawBytes);
...
export function runtimeConfigFromEnv(): RuntimeConfig {
  return {
    apiBaseUrl: process.env.UKPCOM_API_BASE_URL ?? 'https://committees-api.parliament.uk',
    requestTimeoutMs: Number(process.env.UKPCOM_REQUEST_TIMEOUT_MS ?? '10000'),
  };
}
