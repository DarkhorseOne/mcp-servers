...
export async function startHttpServer(port = Number(process.env.UKPCOM_HTTP_PORT ?? '8787')): Promise<import('node:http').Server> {
  ...
  await new Promise<void>((resolve) => {
    server.listen(port, () => {
      console.error(`UK Parliament Committees HTTP proxy listening on :${port}`);
      resolve();
    });
  });
