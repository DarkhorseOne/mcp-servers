...
import { errorToEnvelope, executeEndpoint, matchEndpointByResolvedPath, runtimeConfigFromEnv } from './core.js';
...
export async function startHttpServer(port = Number(process.env.UKPCV_HTTP_PORT ?? '8787')): Promise<void> {
    ...
    if (url.pathname.startsWith('/proxy/')) {
      if (method !== 'GET') {
        writeJsonResponse(res, 405, {
          ...
          error: {
            code: 'INVALID_PARAMS',
            message: 'Only GET is supported for proxied UK Parliament Commons Votes endpoints.',
          },
          upstream_path: '',
          retrieved_at: new Date().toISOString(),
        });
      ...
      try {
        ...
        writeJsonResponse(res, payload.status, payload);
        return;
      } catch (error) {
  ...
  await new Promise<void>((resolve) => {
    server.listen(port, () => {
      console.error(`UK Parliament Commons Votes HTTP proxy listening on :${port}`);
      resolve();
    });
  });
}
