...
function validateTakeLimit(parameter: EndpointParameter, parsedValue: string | number | boolean | string[] | number[]): void {
  if (parameter.name !== 'parameters.take') {
    return;
  }
  ...
  if (parsedValue > 100) {
    throw new EndpointValidationError(`Parameter '${parameter.name}' must be <= 100`, {
      parameter: parameter.name,
      max: 100,
      value: parsedValue,
    });
...
export function buildUpstreamPath(endpoint: EndpointDefinition, rawInput: Record<string, unknown>): string {
  ...
  for (const parameter of endpoint.pathParams) {
    ...
    validateTakeLimit(parameter, parsed);
...
export function runtimeConfigFromEnv(): RuntimeConfig {
  return {
    apiBaseUrl: process.env.UPSTREAM_BASE_URL ?? 'https://oralquestionsandmotions-api.parliament.uk',
