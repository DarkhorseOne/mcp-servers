...
describe('tools registration', () => {
  ...
  it('maps upstream timeout to error payload', async () => {
    ...
    registerAllTools(fakeServer);
    const tool = registered.list_oral_questions;
  ...
  it('includes parameter description and enum options in tool input schema metadata', () => {
    const endpoint = ENDPOINTS.find((entry) => entry.toolName === 'list_early_day_motions');
    ...
    const orderBySchema = properties['parameters.orderBy'] as { description?: string } | undefined;

    expect(orderBySchema?.description).toContain('Order results by date tabled');
    const enumOptions = collectStringEnumOptions(orderBySchema);
    expect(new Set(enumOptions)).toEqual(
      new Set(['DateTabledAsc', 'DateTabledDesc', 'TitleAsc', 'TitleDesc', 'SignatureCountAsc', 'SignatureCountDesc']),
    );
  });
  ...
  it('rejects invalid enum input as INVALID_PARAMS with 400', async () => {
    ...
    registerAllTools(fakeServer);
    const tool = registered.list_early_day_motions;
    expect(tool).toBeDefined();

    const result = (await tool?.({
      'parameters.orderBy': 'NotReal',
  ...
  it('keeps required/optional schema mapping from endpoint metadata', () => {
    ...
    const idConfig = configs.get_early_day_motion_by_id as { inputSchema?: z.ZodTypeAny } | undefined;
    const listConfig = configs.list_oral_questions as { inputSchema?: z.ZodTypeAny } | undefined;
    ...
    expect((listJsonSchema as { required?: string[] }).required ?? []).not.toContain('parameters.take');
  });
});
