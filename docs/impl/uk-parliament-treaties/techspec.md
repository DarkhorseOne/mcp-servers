# UK Parliament Treaties MCP Server 技术规格
...
1. 读取 `${SWAGGER_PATH}`，提取 `paths[*].get` 全量 endpoint（本规格应为 6 个）。
2. 生成 `src/endpoints.generated.ts`：包含 path/method/toolName/summary/params，参数必须带 `description`（swagger 无描述可留空），存在 enum 时写入 `enum`。
...
1. 将 Treaties Swagger 的全部 GET 接口映射为 MCP tools。
...
- 固定：`@darkhorseone/mcp-server-uk-parliament-treaties`
...
- Host：`treaties-api.parliament.uk`
...
从 swagger 提取以下 6 个 GET 接口：

1. `GET /api/BusinessItem/{id}`
2. `GET /api/GovernmentOrganisation`
3. `GET /api/SeriesMembership`
4. `GET /api/Treaty`
5. `GET /api/Treaty/{id}`
6. `GET /api/Treaty/{id}/BusinessItems`
...
- `/api/BusinessItem/{id}` → `get_business_item_by_id`
- `/api/GovernmentOrganisation` → `list_government_organisations`
- `/api/SeriesMembership` → `list_series_memberships`
- `/api/Treaty` → `list_treaties`
- `/api/Treaty/{id}` → `get_treaty_by_id`
- `/api/Treaty/{id}/BusinessItems` → `list_business_items_by_treaty_id`

### 3.6 MCP 工具输入参数规范（按 endpoint）

#### A) `get_business_item_by_id`

- `id` (`string`, required, path)

#### B) `list_government_organisations`

- 无参数

#### C) `list_series_memberships`
...
- `SearchText` (`string`)
- `GovernmentOrganisationId` (`number`)
- `Series` (`string`, enum from `SeriesMembershipType`)
- `ParliamentaryProcess` (`string`, enum from `ParliamentaryProcess`)
- `DebateScheduled` (`boolean`)
- `MotionsTabledAboutATreaty` (`boolean`)
- `CommitteeRaisedConcerns` (`boolean`)
...
#### F) `list_business_items_by_treaty_id`
...
该 swagger 的响应模型与 schema 层级较多（Resource / Collection 封装），实现必须遵循：
...
```json
{
  "scripts": {
    ...
    "check:tarball:shebang": "node ./scripts/add-shebang.mjs && node ./scripts/verify-tarball-shebang.mjs",
    ...
    "inspect:npm": "npx -y @modelcontextprotocol/inspector npx -y -p @darkhorseone/mcp-server-uk-parliament-treaties mcp-server-uk-parliament-treaties"
  }
}
```
...
- `/healthz` 正常返回工具数量。
- path 参数映射正确（例如 `/api/Treaty/{id}`）。
- list 接口数组 query 序列化为 multi 重复键（若后续扩展存在数组参数）。
...
`servers/uk-parliament-treaties/README.md` 至少包含：
...
- swagger 响应封装模型兼容策略说明（以实际上游返回为准）
...
1. swagger 的 6 个 GET endpoint 均已生成并可调用。
2. 每个 endpoint 的参数 required/type/description/enum 映射正确。
3. stdio MCP server 可启动并列出 6 个 tools。
...
- `docs/guide.md`
- `docs/impl/uk-parliament-treaties/swagger.json`
