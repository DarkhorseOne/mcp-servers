# 发布到 MCP Registry（uk-parliament-members）操作步骤文档（仅计划，不执行）

> 目的：依据 `docs/publish-to-mcp-registry-guide.md`，制定 **@darkhorseone/mcp-server-uk-parliament-members** 发布到官方 MCP Registry 的完整行动计划。
...
- 目标包：`@darkhorseone/mcp-server-uk-parliament-members`
- MCP Registry 名称（mcpName）：`io.github.nick-ma/uk-parliament-members`
  - 说明：使用 GitHub 登录时，`mcpName` **必须** 以 `io.github.nick-ma/` 开头。
- 版本：与 npm 已发布版本保持一致（当前本仓库包版本为 `1.0.1`）。
...
1. 在 `servers/uk-parliament-members/package.json` 增加：
   ```json
   {
     "mcpName": "io.github.nick-ma/uk-parliament-members"
   }
   ```
...
4. 发布到 npm
   ```bash
   pnpm --filter @darkhorseone/mcp-server-uk-parliament-members publish --access public
   ```
...
1. 进入包目录
   ```bash
   cd servers/uk-parliament-members
   ```
2. 生成模板
   ```bash
   mcp-publisher init
...
3. 编辑 `server.json`（示例结构）
   ```json
   {
     ...
     "name": "io.github.nick-ma/uk-parliament-members",
     "description": "MCP server for UK Parliament Members API",
     "repository": {
     ...
     "packages": [
       {
         "registryType": "npm",
         "identifier": "@darkhorseone/mcp-server-uk-parliament-members",
         "version": "1.0.1",
         "transport": {
           "type": "stdio"
         }
...
```bash
curl "https://registry.modelcontextprotocol.io/v0.1/servers?search=io.github.nick-ma/uk-parliament-members"
```
