# 发布到 MCP Registry（uk-parliament-committees）操作步骤文档（仅计划，不执行）

> 目的：依据 `docs/publish-to-mcp-registry-guide.md`，制定 **@darkhorseone/mcp-server-uk-parliament-committees** 发布到官方 MCP Registry 的完整行动计划。
...
- 目标包：`@darkhorseone/mcp-server-uk-parliament-committees`
- MCP Registry 名称（mcpName）：`io.github.nick-ma/uk-parliament-committees`
...
1. 在 `servers/uk-parliament-committees/package.json` 增加：
   ```json
   {
     "mcpName": "io.github.nick-ma/uk-parliament-committees"
   }
   ```
...
4. 发布到 npm
   ```bash
   pnpm --filter @darkhorseone/mcp-server-uk-parliament-committees publish --access public
   ```
...
1. 进入包目录
   ```bash
   cd servers/uk-parliament-committees
   ```
2. 生成模板
   ```bash
   mcp-publisher init
...
3. 编辑 `server.json`（示例结构）
   ```json
   {
     ...
     "name": "io.github.nick-ma/uk-parliament-committees",
     "description": "MCP server for UK Parliament Committees API",
     "repository": {
     ...
     "packages": [
       {
         "registryType": "npm",
         "identifier": "@darkhorseone/mcp-server-uk-parliament-committees",
         "version": "1.0.0",
         "transport": {
           "type": "stdio"
         }
...
```bash
curl "https://registry.modelcontextprotocol.io/v0.1/servers?search=io.github.nick-ma/uk-parliament-committees"
```
