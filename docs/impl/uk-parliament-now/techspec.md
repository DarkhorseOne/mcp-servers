# UK Parliament NOW (Annunciator Content) MCP Server 技术规格
...
1. 读取 `${SWAGGER_PATH}`，提取 `paths[*].get` 全量 endpoint（本规格应为 2 个）。
...
1. 将 NOW Swagger 的全部 GET 接口映射为 MCP tools。
...
- 固定：`@darkhorseone/mcp-server-uk-parliament-now`
...
从 swagger 提取以下 2 个 GET 接口：

1. `GET /api/Message/message/{annunciator}/current`
2. `GET /api/Message/message/{annunciator}/{date}`
...
- 参数 schema 使用 `$ref` 且目标是 enum（例如 `AnnunciatorMessageType`）时，映射为 `string` 且必须带 `enum` 列表。
...
- 对 `$ref` enum（`#/components/schemas/AnnunciatorMessageType`）必须解析得到 `['CommonsMain', 'LordsMain']`。
...
- `/api/Message/message/{annunciator}/current` → `get_current_message_by_annunciator`
- `/api/Message/message/{annunciator}/{date}` → `get_latest_message_by_annunciator_and_date`

### 3.6 MCP 工具输入参数规范（按 endpoint）

#### A) `get_current_message_by_annunciator`

- `annunciator` (`string`, required, path, enum: `CommonsMain|LordsMain`)

#### B) `get_latest_message_by_annunciator_and_date`

- `annunciator` (`string`, required, path, enum: `CommonsMain|LordsMain`)
- `date` (`string`, required, path, format: `date-time`)
...
该 swagger 在 `components.schemas` 定义了大量对象与 enum（Message/Slide/Line 等），实现必须遵循：
...
- enum 参数（本规格中的 `annunciator`）必须使用 `z.enum([...])` 或等价严格枚举约束。
...
- 断言 `annunciator` 参数含 enum（`CommonsMain`, `LordsMain`）。

### 8.2 `test/tools.generated.test.ts`

- 断言注册工具数量与 endpoint 一致（2）。
- 断言 path 参数在 inputSchema 中为必填。
- 断言 enum 在 schema 中可观察（JSON schema 或 parse 行为）。
- 断言非法 `annunciator` 输入映射为 `INVALID_PARAMS` + 400。

### 8.3 `test/http.proxy.test.ts`

- `/healthz` 返回 200。
- 未匹配 endpoint 返回 404。
- path 参数映射正确（示例：`/proxy/api/Message/message/CommonsMain/current`）。
- 第二个 endpoint 的双 path 参数映射正确（示例：`/proxy/api/Message/message/LordsMain/2025-01-01T00:00:00Z`）。
...
`servers/uk-parliament-now/README.md` 至少包含：

1. 项目简介（对应 Annunciator Content API）。
2. 安装与构建命令。
...
1. endpoint 覆盖完整：swagger 的 2 个 GET 全部映射为 MCP tools。
2. toolName 稳定且唯一，测试中有断言。
3. `annunciator` enum 约束落地到 endpoint metadata + inputSchema。
4. stdio 与 HTTP 两入口都可运行。
5. 错误映射符合本规格第 4.5 节。
6. `pnpm --filter @darkhorseone/mcp-server-uk-parliament-now run check` 通过。
...
pnpm --filter @darkhorseone/mcp-server-uk-parliament-now run check
pnpm --filter @darkhorseone/mcp-server-uk-parliament-now run check:tarball:shebang
pnpm run check
```
...
- 规范指南：`docs/guide.md`
- API 描述：`docs/impl/uk-parliament-now/swagger.json`
- OpenAPI 信息：`Annunciator content API` / version `v1`
