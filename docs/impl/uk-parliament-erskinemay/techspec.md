# UK Parliament Erskine May MCP Server 技术规格
...
```bash
SERVER_SLUG="uk-parliament-erskinemay"
PACKAGE_NAME="@darkhorseone/mcp-server-${SERVER_SLUG}"
PACKAGE_DIR="servers/${SERVER_SLUG}"
IMPL_DIR="docs/impl/uk-parliament-erskinemay"
...
1. 读取 `${SWAGGER_PATH}`，提取 `paths[*].get` 全量 endpoint（本规格应为 11 个）。
...
1. 将 Erskine May Swagger 的全部 GET 接口映射为 MCP tools。
...
- 固定：`@darkhorseone/mcp-server-uk-parliament-erskinemay`
...
- Host：`erskinemay-api.parliament.uk`
- Schemes：默认使用 `https`
...
从 swagger 提取以下 11 个 GET 接口：

1. `GET /api/Chapter/{chapterNumber}`
2. `GET /api/IndexTerm/browse`
3. `GET /api/IndexTerm/{indexTermId}`
4. `GET /api/Part`
5. `GET /api/Part/{partNumber}`
6. `GET /api/Search/IndexTermSearchResults/{searchTerm}`
7. `GET /api/Search/Paragraph/{reference}`
8. `GET /api/Search/ParagraphSearchResults/{searchTerm}`
9. `GET /api/Search/SectionSearchResults/{searchTerm}`
10. `GET /api/Section/{sectionId}`
11. `GET /api/Section/{sectionId},{step}`
...
- 本 swagger 参数层无 enum；若后续出现 enum，必须写入 endpoint metadata 并下沉到 inputSchema。
...
- 本 swagger 参数无 enum；若未来 swagger 增加 enum，生成逻辑必须自动约束。
...
- `/api/Chapter/{chapterNumber}` → `get_chapter_by_chapter_number`
- `/api/IndexTerm/browse` → `browse_index_terms`
- `/api/IndexTerm/{indexTermId}` → `get_index_term_by_index_term_id`
- `/api/Part` → `list_parts`
- `/api/Part/{partNumber}` → `get_part_by_part_number`
- `/api/Search/IndexTermSearchResults/{searchTerm}` → `search_index_term_results_by_search_term`
- `/api/Search/Paragraph/{reference}` → `get_paragraph_by_reference`
- `/api/Search/ParagraphSearchResults/{searchTerm}` → `search_paragraph_results_by_search_term`
- `/api/Search/SectionSearchResults/{searchTerm}` → `search_section_results_by_search_term`
- `/api/Section/{sectionId}` → `get_section_by_section_id`
- `/api/Section/{sectionId},{step}` → `get_section_overview_by_section_id_and_step`

### 3.6 MCP 工具输入参数规范（按 endpoint）

#### A) `get_chapter_by_chapter_number`

- `chapterNumber` (`number`, required, path)

#### B) `browse_index_terms`

- `startLetter` (`string`, query)
- `skip` (`number`, query, default 0)
- `take` (`number`, query, default 20)

#### C) `get_index_term_by_index_term_id`

- `indexTermId` (`number`, required, path)

#### D) `list_parts`

- 无参数

#### E) `get_part_by_part_number`

- `partNumber` (`number`, required, path)

#### F) `search_index_term_results_by_search_term`

- `searchTerm` (`string`, required, path)
- `skip` (`number`, query, default 0)
- `take` (`number`, query, default 20)

#### G) `get_paragraph_by_reference`

- `reference` (`string`, required, path)

#### H) `search_paragraph_results_by_search_term`

- `searchTerm` (`string`, required, path)
- `skip` (`number`, query, default 0)
- `take` (`number`, query, default 20)

#### I) `search_section_results_by_search_term`

- `searchTerm` (`string`, required, path)
- `skip` (`number`, query, default 0)
- `take` (`number`, query, default 20)

#### J) `get_section_by_section_id`

- `sectionId` (`number`, required, path)

#### K) `get_section_overview_by_section_id_and_step`

- `sectionId` (`number`, required, path)
- `step` (`number`, required, path)
...
该 swagger 在 `components.schemas` 定义了多层对象（Overview / Detail / SearchResult），实现必须遵循：
...
- 本 swagger 参数无 enum，未来如出现 enum 必须使用 `z.enum([...])`。
...
- 断言关键参数类型正确（如 `chapterNumber:number`、`searchTerm:string`、`step:number`）。

### 8.2 `test/tools.generated.test.ts`

- 断言注册工具数量与 endpoint 一致（11）。
- 断言 required/path 参数在 inputSchema 中为必填。
- 断言 `skip/take` 等可选查询参数为 optional。

### 8.3 `test/http.proxy.test.ts`

- `/healthz` 返回 200。
- 未匹配 endpoint 返回 404。
- path 参数映射正确（示例：`/proxy/api/Section/12,1`）。
- query 参数透传正确（示例：`skip/take/startLetter`）。
...
`servers/uk-parliament-erskinemay/README.md` 至少包含：

1. 项目简介（对应 Erskine May API）。
2. 安装与构建命令。
...
1. endpoint 覆盖完整：swagger 的 11 个 GET 全部映射为 MCP tools。
2. toolName 稳定且唯一，测试中有断言。
3. stdio 与 HTTP 两入口都可运行。
4. 错误映射符合本规格第 4.5 节。
5. `pnpm --filter @darkhorseone/mcp-server-uk-parliament-erskinemay run check` 通过。
6. `check:tarball:shebang` 通过（bin 产物具备 shebang）。
...
pnpm --filter @darkhorseone/mcp-server-uk-parliament-erskinemay run check
pnpm --filter @darkhorseone/mcp-server-uk-parliament-erskinemay run check:tarball:shebang
pnpm run check
```
...
- 规范指南：`docs/guide.md`
- API 描述：`docs/impl/uk-parliament-erskinemay/swagger.json`
- OpenAPI 信息：`Erskine May API` / version `v1`
