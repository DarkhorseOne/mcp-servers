...
## Purpose
Find nearby pharmacy services for a UK postcode.

## Technical implementation details
- Tool name: `find_pharmacies`
...
- Response mapping:
  - Returns `search_postcode`, `total_results`, `pharmacies`, `data_source`, `retrieved_at`
...
async function example(client: {
  ...
  const result = await client.callTool('find_pharmacies', { postcode: 'SW1A 1AA' })
